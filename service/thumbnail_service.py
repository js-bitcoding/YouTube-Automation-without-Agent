...
# from thumbnails_utils.db import SessionLocal, Thumbnail
# from config import GENERATED_THUMBNAILS_PATH

# os.makedirs(GENERATED_THUMBNAILS_PATH, exist_ok=True)

# def apply_random_effects(image_path):
#     """Applies random effects to create variations."""
#     img = cv2.imread(image_path)
    
#     # Random effect: Adjust brightness & contrast
#     alpha = random.uniform(0.8, 1.2)  # Contrast
#     beta = random.randint(-30, 30)    # Brightness
#     img = cv2.convertScaleAbs(img, alpha=alpha, beta=beta)

#     # Random effect: Overlay color tint
#     if random.choice([True, False]):
#         tint = np.full(img.shape, (random.randint(50, 150), 0, 0), dtype=np.uint8)
#         img = cv2.addWeighted(img, 0.7, tint, 0.3, 0)

#     return img

# def add_text_overlay(image_path, text="Best AI Tool!"):
#     """Adds a text overlay to the thumbnail."""
...
# def generate_new_thumbnails():
#     """Generates 3 different thumbnails from the top 10."""
#     db = SessionLocal()
#     thumbnails = db.query(Thumbnail).limit(10).all()
    
#     if len(thumbnails) < 3:
#         return {"error": "Not enough thumbnails available."}
    
#     selected = random.sample(thumbnails, 3)
#     generated_paths = []
    
#     for i, thumb in enumerate(selected):
#         img = apply_random_effects(thumb.saved_path)  # Apply effects
#         edited_img = add_text_overlay(thumb.saved_path)  # Add text

#         # Save new thumbnail
#         output_path = os.path.join(GENERATED_THUMBNAILS_PATH, f"generated_{i+1}.jpg")
#         edited_img.save(output_path)
#         generated_paths.append(output_path)
    
#     db.close()
#     return {"generated_thumbnails": generated_paths}
